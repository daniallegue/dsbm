...
# NOTE (dsbm): default changed from 'share/4AA_sims_replica' to None, and the
# replica section below (an auxiliary "vs naive short MD" baseline, unrelated to
# the gen_prob/gen_valid_rate/gen_JSD metrics against the reference MSM) made
# conditional on it being set -- see vendor/mdgen/PROVENANCE.md. Behavior is
# unchanged whenever --repdir IS given.
parser.add_argument('--repdir', type=str, default=None)
...
def main(name):
    ...
    ### Replica analysis (skipped entirely if --repdir wasn't given -- see NOTE at
    ### the --repdir argparse definition above)
    ...
    # NOTE (dsbm): wrapped in try/except -- diagnostic-only (the generated-ensemble
    # flux plot, not any of the gen_prob/gen_valid_rate/gen_JSD metrics, which are
    # already computed above by this point). Fitting an MSM+TPT on a small or
    # low-quality generated ensemble can raise a KeyError when the generated paths
    # don't cleanly revisit both endpoint states in gen_tps_msm's active set; without
    # this guard that crash happens *before* the output .pkl is ever written, losing
    # the metrics above too. See vendor/mdgen/PROVENANCE.md.
    try:
        ...
        print(f'skipping generated-ensemble flux plot for {name}: {e}')
        axs[1, 3].set_title('generated flux plot skipped (see log)')
