...
if args.atlas:
    def do_job(name):
        for i in [1,2,3]:
            # NOTE (dsbm): upstream referenced the undefined `args.atlas_dir`; this
            # script only ever defines `--sim_dir`, so fixed to match -- see
            # vendor/mdgen/PROVENANCE.md. Purely a dead-reference fix, no logic change.
    ...
    def do_job(name):
        # NOTE (dsbm): same `args.atlas_dir` -> `args.sim_dir` fix as above.
